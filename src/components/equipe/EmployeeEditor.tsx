    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Data de admissão">
          <Input type="date" value={f.hire_date} onChange={upd("hire_date")} />
        </Field>
        <Field label="Data de rescisão">
          <Input type="date" value={f.termination_date} onChange={upd("termination_date")} />
        </Field>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="NIF">
          <Input value={f.tax_id_nif} onChange={upd("tax_id_nif")} />
        </Field>
        <Field label="NISS">
          <Input value={f.social_security_niss} onChange={upd("social_security_niss")} />
        </Field>
      </div>
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Field label="Estado civil">
          <Select
            value={f.marital_status || "none"}
            onValueChange={(v) => setF((s) => ({ ...s, marital_status: v === "none" ? "" : v }))}
          >
            <SelectTrigger>
              <SelectValue placeholder="—" />
            </SelectTrigger>
        ...
        <Field label="Nº dependentes">
          <Input type="number" min="0" value={f.dependents_count} onChange={upd("dependents_count")} />
        </Field>
      </div>
      <div className="grid gap-3 sm:grid-cols-3">
        <Field label="Tipo de contrato">
          <Select
            value={f.contract_type || "none"}
            onValueChange={(v) => setF((s) => ({ ...s, contract_type: v === "none" ? "" : v }))}
          >
            <SelectTrigger>
              <SelectValue placeholder="—" />
            </SelectTrigger>
        ...
        <Field label="Horas contratadas (semana)">
          <Input type="number" step="0.5" value={f.weekly_contracted_hours} onChange={upd("weekly_contracted_hours")} />
        </Field>
      ...
      <details className="rounded-lg border border-border/60 bg-muted/30 p-3">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <Field label="Nacionalidade">
            <Input value={f.nationality} onChange={upd("nationality")} placeholder="Ex.: Portuguesa" />
          </Field>
          <Field label="País fiscal">
            <Input value={f.tax_country} onChange={upd("tax_country")} placeholder="Ex.: PT" />
          </Field>
        </div>
      </details>
  ...
  return (
    ...
    >
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Field label="Número">
          <Input value={f.main_doc_number} onChange={upd("main_doc_number")} />
        </Field>
        <Field label="Validade">
          <Input type="date" value={f.main_doc_expires_at} onChange={upd("main_doc_expires_at")} />
        </Field>
      </div>
      ...
      <DocPair
        label="A1"
        num={f.a1_number}
        date={f.a1_expires_at}
        onNum={upd("a1_number")}
        onDate={upd("a1_expires_at")}
      />
      <DocPair
        label="Carta de condução"
        num={f.driver_license_number}
        date={f.driver_license_expires_at}
        onNum={upd("driver_license_number")}
        onDate={upd("driver_license_expires_at")}
      />
      <DocPair
        label="Passaporte"
        num={f.passport_number}
        date={f.passport_expires_at}
        onNum={upd("passport_number")}
        onDate={upd("passport_expires_at")}
      />
      <DocPair
        label="Cartão de saúde"
        num={f.health_card_number}
        date={f.health_card_expires_at}
        onNum={upd("health_card_number")}
        onDate={upd("health_card_expires_at")}
      />
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Medicina trabalho — última">
          <Input type="date" value={f.occ_health_last_at} onChange={upd("occ_health_last_at")} />
        </Field>
        <Field label="Medicina trabalho — próxima">
...
function DocPair({
  ...
  num: string;
  date: string;
  onNum: (e: { target: { value: string } }) => void;
  onDate: (e: { target: { value: string } }) => void;
}) {
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      <div className="text-sm font-medium pt-7 text-muted-foreground">{label}</div>
      <Field label="Número">
        <Input value={num} onChange={onNum} />
      </Field>
      <Field label="Validade">
...
function TabFinanceiro({
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="IBAN">
          <Input value={f.iban} onChange={upd("iban")} />
        </Field>
        <Field label="SWIFT">
          <Input value={f.swift} onChange={upd("swift")} />
        </Field>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Valor hora — semana">
          <Input type="number" step="0.01" value={f.rate_hour_week} onChange={upd("rate_hour_week")} />
        </Field>
        <Field label="Valor hora — fim de semana/feriado">
          <Input type="number" step="0.01" value={f.rate_hour_weekend} onChange={upd("rate_hour_weekend")} />
        </Field>
        <Field label="Valor dia — Bélgica">
          <Input type="number" step="0.01" value={f.rate_day_be} onChange={upd("rate_day_be")} />
        </Field>
        <Field label="Valor dia — estrangeiro">
          <Input type="number" step="0.01" value={f.rate_day_foreign} onChange={upd("rate_day_foreign")} />
        </Field>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Alimentação">
          <Input type="number" step="0.01" value={f.allowance_meal} onChange={upd("allowance_meal")} />
        </Field>
        <Field label="Transporte">
          <Input type="number" step="0.01" value={f.allowance_transport} onChange={upd("allowance_transport")} />
        </Field>
        <Field label="Renda">
          <Input type="number" step="0.01" value={f.allowance_rent} onChange={upd("allowance_rent")} />
        </Field>
        <Field label="Outros">
          <Input type="number" step="0.01" value={f.allowance_other} onChange={upd("allowance_other")} />
        </Field>
      </div>
...
function TabAssinaturas({
  ...
  const upload = async (kind: "sig" | "ini", file: File) => {
    ...
    try {
      ...
      if (kind === "sig") setSignatureUrl(path);
      else setInitialsUrl(path);
    } catch (err) {
      toast.error((err as Error).message);
    } finally {
      setBusy(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <SignatureSlot
        ...
        path={signatureUrl}
        loading={busy === "sig"}
        onPick={(f) => upload("sig", f)}
      />
      <SignatureSlot
        label="Visto digital (rubrica)"
        path={initialsUrl}
        loading={busy === "ini"}
        onPick={(f) => upload("ini", f)}
      />
    </div>
  );
}
  ...
  const { data: items = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (
        supabase.from("employee_attachments" as never) as unknown as {
          select: (s: string) => {
            eq: (
              c: string,
              v: string,
        ...
        {items.map((it) => (
          <li key={it.id} className="flex items-center justify-between gap-3 px-4 py-2 text-sm">
            ...
            <div className="flex shrink-0 gap-1">
              <Button size="icon" variant="ghost" onClick={() => onDownload(it)} title="Baixar">
                <Download className="h-4 w-4" />
              </Button>
              <Button size="icon" variant="ghost" onClick={() => onDelete(it)} title="Remover">
