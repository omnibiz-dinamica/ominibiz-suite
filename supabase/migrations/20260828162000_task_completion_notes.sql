-- SUP-2026-000112: completion observations reuse the task audit timeline.
-- The constraint is extended additively; existing audit values remain valid.
...
ALTER TABLE public.task_audit_events
  ...
  CHECK (event IN ('cancel','archive','unarchive','absence','delete','series_end','completion_note'));
...
BEGIN
  ...
  -- Idempotencia contra duplo clique/retry da mesma observacao.
