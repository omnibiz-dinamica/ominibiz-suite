-- Corrige erro "function round(double precision, integer) does not exist" na
-- deteção de tickets semelhantes: o score deriva de similarity() (real), pelo
-- que precisa de cast explícito para numeric antes do round.
CREATE OR REPLACE FUNCTION public.support_find_similar(_company_id uuid, _type support_ticket_type, _title text, _description text, _module text DEFAULT NULL::text, _route text DEFAULT NULL::text, _limit integer DEFAULT 5, _destination_code text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
REVOKE EXECUTE ON FUNCTION public.support_find_similar(uuid, support_ticket_type, text, text, text, text, integer, text) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.support_find_similar(uuid, support_ticket_type, text, text, text, text, integer, text) TO authenticated, service_role;