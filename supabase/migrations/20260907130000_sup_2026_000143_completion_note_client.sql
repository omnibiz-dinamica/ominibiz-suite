-- SUP-2026-000143: inclui o cliente na notificacao canonica de conclusao.
-- A observacao continua sendo persistida apenas em task_audit_events.
...
BEGIN
  ...
  SELECT * INTO v_existing
   ...
   WHERE task_id = v_task.id AND event = 'completion_note'
     AND actor_user_id = v_uid AND reason = v_note
  ...
  UPDATE public.notifications n
     SET title = v_title, body = v_body, priority = 'media',
         metadata = COALESCE(n.metadata, '{}'::jsonb) || v_metadata
   WHERE n.company_id = v_task.company_id
     AND n.task_id = v_task.id AND n.event = 'task_completed';

  FOR v_mgr IN
    SELECT DISTINCT user_id FROM public.user_roles
     WHERE company_id = v_task.company_id
       AND role IN ('manager', 'owner') AND user_id <> v_uid
  LOOP
