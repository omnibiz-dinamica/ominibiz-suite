-- Férias aprovadas são eventos administrativos: aparecem na Gestão de Ponto
-- sem criar time_entries fictícios nem alterar a carga efetivamente trabalhada.
CREATE INDEX IF NOT EXISTS idx_vacation_requests_operational_feed
  ON public.vacation_requests (company_id, status, start_date, end_date, user_id);
...
BEGIN
  ...
  RETURN (
    WITH work_rows AS (
      ...
      SELECT md5(vr.id::text || ':' || vacation_day.work_date::text)::uuid AS id,
        vr.company_id, NULL::uuid AS task_id, vr.user_id, NULL::timestamptz AS started_at,
        NULL::timestamptz AS ended_at, NULL::timestamptz AS paused_at, NULL::timestamptz AS resumed_at,
        NULL::integer AS effective_minutes, COALESCE(NULLIF(btrim(vr.note), ''), 'Férias aprovadas') AS notes,
        ...
        vacation_day.work_date::timestamptz AS scheduled_for, NULL::timestamptz AS scheduled_end,
        vacation_day.work_date::date AS recurrence_date, NULL::timestamptz AS due_at,
        ...
        vacation_day.work_date::timestamptz AS sort_at
      FROM public.vacation_requests vr
      JOIN public.profiles p ON p.id = vr.user_id
      CROSS JOIN LATERAL generate_series(
        GREATEST(vr.start_date, COALESCE(_from_date, vr.start_date))::timestamp,
        LEAST(vr.end_date, COALESCE(_to_date, vr.end_date))::timestamp,
        interval '1 day'
      ) AS vacation_day(work_date)
