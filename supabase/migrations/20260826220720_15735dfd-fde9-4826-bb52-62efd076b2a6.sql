...
BEGIN
  ...
  LOOP
    ...
    IF v_history OR v_t.status IN ('em_andamento', 'concluido') THEN
      -- Historico operacional: nunca apagar. Cancelar quando ainda for possivel.
...
GRANT EXECUTE ON FUNCTION public.task_series_delete(uuid, text, text) TO service_role;