-- SUP-2026-000131
-- Mantem a edicao de series recorrentes no mesmo contrato wall-clock usado
-- pela materializacao: a data da ocorrencia e o dia de inicio e a duracao
-- pode levar o scheduled_end para o dia seguinte.
  ...
  UPDATE public.tasks t SET
    ...
    scheduled_for = CASE
      WHEN v_has_time AND t.recurrence_date IS NOT NULL
        THEN ((t.recurrence_date::text || ' ' || v_new_time::text)::timestamp AT TIME ZONE 'UTC')
      ELSE t.scheduled_for END,
    scheduled_end = CASE
      WHEN (v_has_time OR v_has_dur) AND t.recurrence_date IS NOT NULL
        THEN ((t.recurrence_date::text || ' ' || v_new_time::text)::timestamp AT TIME ZONE 'UTC')
             + make_interval(mins => v_new_dur)
