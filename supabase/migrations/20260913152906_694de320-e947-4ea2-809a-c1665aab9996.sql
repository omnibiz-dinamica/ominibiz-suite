ALTER TABLE public.tasks
  ADD COLUMN IF NOT EXISTS schedule_change_suggested_employee_name text;
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    ELSIF NEW.status = 'cancelado' AND NEW.refused_by IS NOT NULL AND OLD.refused_by IS NULL THEN
      ...
      v_schedule_suffix := CASE WHEN NEW.schedule_change_requested_date IS NOT NULL
        ...
        || CASE WHEN NULLIF(btrim(NEW.schedule_change_suggested_employee_name), '') IS NOT NULL
          THEN ' Sugestao: ' || btrim(NEW.schedule_change_suggested_employee_name) || '.' ELSE '' END;
      FOR v_mgr IN
      ...
      LOOP
        PERFORM public._notify(NEW.company_id, v_mgr.user_id, NEW.id,
          ...
          jsonb_build_object(
            ...
            'schedule_change_suggested_employee_name', NEW.schedule_change_suggested_employee_name,
...
DROP FUNCTION IF EXISTS public.task_cancel_with_schedule_request(uuid, text, date, boolean, uuid, time);
CREATE OR REPLACE FUNCTION public.task_cancel_with_schedule_request(
  ...
  _suggested_employee_name text DEFAULT NULL
)
...
BEGIN
  ...
  UPDATE public.tasks
     SET schedule_change_requested_date = _requested_date,
         ...
         schedule_change_suggested_employee_name = _suggested_employee_name,
         updated_at = now()
   WHERE id = _task_id;
...
DROP FUNCTION IF EXISTS public.task_transition_with_schedule_request(uuid, text, text, date, boolean, uuid, time);
...
BEGIN
  ...
  UPDATE public.notifications n
     SET metadata = COALESCE(n.metadata, '{}'::jsonb) || jsonb_build_object(
           ...
           'schedule_change_needs_reassignment', _needs_reassignment,
           'schedule_change_suggested_employee_id', _suggested_employee_id,
           'schedule_change_suggested_employee_name', COALESCE(_suggested_employee_name, v_suggested),
...
REVOKE ALL ON FUNCTION public.task_cancel_with_schedule_request(uuid, text, date, boolean, uuid, time, text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.task_transition_with_schedule_request(uuid, text, text, date, boolean, uuid, time, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.task_cancel_with_schedule_request(uuid, text, date, boolean, uuid, time, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.task_transition_with_schedule_request(uuid, text, text, date, boolean, uuid, time, text) TO authenticated;
